...
{
    model.RowHeights[r - 1] =
        ExcelRowHeightToPixels(ws.Row(r).Height);
}
...
private double ExcelColumnWidthToPixels(double width)
{
    ...
    return Math.Floor((256 * width + 5) / 256 * 7);
}

private double ExcelRowHeightToPixels(double heightInPoints)
{
    double px = heightInPoints * 96.0 / 72.0;
    return px > 0 ? px : 20; // minimalna wysokość wiersza
}
